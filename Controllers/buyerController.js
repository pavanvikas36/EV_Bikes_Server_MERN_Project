...
exports.addToWishlistById = async (req, res) => {
  try {
    ...
    // Check if already in wishlist using vehicleId
    const existingItem = buyer.wishlist.find(
      item => item.vehicleId.toString() === vehicleId
    );
    ...
    // Add to wishlist with proper structure
    buyer.wishlist.push({
      vehicleId: vehicle._id,
      brand: vehicle.brand,
      model: vehicle.model,
...
exports.viewAllWishlist = async (req, res) => {
  try {
    ...
    const buyer = await BuyerModel.findById(buyerId).select("wishlist");
    if (!buyer) {
      return res.status(404).json({ message: "Buyer Not Found" });
    }
...
exports.removeFromWishlistById = async (req, res) => {
  try {
    ...
    const initialLength = buyer.wishlist.length;
    buyer.wishlist = buyer.wishlist.filter(
      item => item.vehicleId.toString() !== vehicleId
    );

    if (buyer.wishlist.length === initialLength) {
      return res.status(404).json({ message: "Vehicle Not Found In Wishlist" });
    }
